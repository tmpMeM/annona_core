...
import (
	...
	model "github.com/AnnonaOrg/annona_core/model/keyword_info"
...
func Delete(c *gin.Context) {
	...
	if len(userInfoHash) > 0 {
		go load_db2redis.DelKeyword(userInfoHash, u.KeyWorld)
	}
}
...
func DeleteAll(c *gin.Context) {
	...
	// 同步删除缓存
	if len(userInfoHash) > 0 {
		go load_db2redis.DelAllKeyword(userInfoHash)
	}
}
