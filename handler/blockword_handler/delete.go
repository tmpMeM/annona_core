...
import (
	...
	model "github.com/AnnonaOrg/annona_core/model/blockword_info"
...
func Delete(c *gin.Context) {
	...
	if len(userInfoHash) > 0 {
		go load_db2redis.DelBlockword(userInfoHash, u.KeyWorld)
	}
}
...
func DeleteAll(c *gin.Context) {
	...
	// 同步删除缓存
	if len(userInfoHash) > 0 {
		go load_db2redis.DelAllBlockword(userInfoHash)
	}
}
